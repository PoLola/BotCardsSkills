...
{
    // Represents a bot that processes incoming activities. For each user interaction, an instance of this class is created and the OnTurnAsync method is called. This is a Transient lifetime service. Transient lifetime services are created
    // each time they're requested. For each Activity received, a new instance of this class is created. Objects that are expensive to construct, or have a lifetime beyond the single turn, should be carefully managed.    //
    // For example, the "MemoryStorage" object and associatedIStatePropertyAccessor{T} object are created with a singleton lifetime.
    public class WelcomeUserBot : ActivityHandler
    {
        ...
        public WelcomeUserBot(UserState userState, IMailService mailService, AppSettings appSetting)//, IAppServices appServices
        { 
...
// another channel the reason is most likely that the channel does not send this activity.
        ...
        private async Task SendHiCardAsync(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            var card = new HeroCard
            {
                ...
                Images = new List<CardImage>() { new CardImage("https://i.ibb.co/CQDTZLy/hola.gif") },
        ...
        private static async Task SendEmailCardAsync(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            var card = new HeroCard
            {
                ...
                Images = new List<CardImage>() { new CardImage("https://w7.pngwing.com/pngs/185/778/png-transparent-thumb-signal-thumb-up-text-hand-presentation-thumbnail.png") },
        ...
        private static async Task SmileCardAsync(ITurnContext turnContext, CancellationToken cancellationToken)
        {
            var card = new HeroCard
            { 
                Images = new List<CardImage>() { new CardImage("https://static.thenounproject.com/png/415504-200.png") },
            };
